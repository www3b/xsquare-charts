...
import {_alignStartEnd, _textX, _toLeftRightCenter} from '../shared/extras.js';
...
export class Legend extends Element {
  ...
  buildLabels() {
    ...
    if (labelOpts.filter) {
      legendItems = legendItems.filter((item) => labelOpts.filter(item, this.chart._data));
    }

    if (labelOpts.sort) {
      legendItems = legendItems.sort((a, b) => labelOpts.sort(a, b, this.chart._data));
    }
  ...
  _fitRows(titleHeight, fontSize, boxWidth, itemHeight) {
    ...
    this.legendItems.forEach((legendItem, i) => {
      const itemWidth = boxWidth + (fontSize / 2) + this.chart._renderer.measureText(legendItem.text, toFont(this.options.labels.font).string);
  ...
  _fitCols(titleHeight, labelFont, boxWidth, _itemHeight) {
    ...
    this.legendItems.forEach((legendItem, i) => {
      const {itemWidth, itemHeight} = calculateItemSize(boxWidth, labelFont, this.chart._renderer.measureText.bind(this.chart._renderer), legendItem, _itemHeight);
  ...
  draw() {
    this.chart._renderer.drawLegend(this);
  }
  ...
  defaults: {
    ...
    labels: {
      ...
      generateLabels(chart) {
        const datasets = chart._data.datasets;
