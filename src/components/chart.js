...
import {createContext, _isPointInArea, getDatasetClipArea} from '../shared/index.js';
import {serializeSvgChart} from '../renderers/svg/export.js';
import {debounce} from '../shared/extras.js';

/**
 * @typedef { import('../shared/types/index.js').ChartEvent } ChartEvent
 * @typedef { import('../shared/types/index.js').Point } Point
 */
...
const instances = {};
const getChart = (key) => {
  return Object.values(instances).filter((c) => c.host === key || c.canvas === key || c._renderer && c._renderer.root === key).pop();
};
...
class Chart {
  ...
  get data() {
    ...
    return {labels: data.labels, series: data.datasets};
  }

  set data(data) {
    this.config.data = normalizeChartData(data);
  }
  ...
  get surface() {
    return this._renderer && (this._renderer.root || this._renderer.canvas) || null;
  }

  get renderer() {
    return this._renderer && this._renderer.type || null;
  }
  ...
  isSeriesVisible(index) {
    return this.isDatasetVisible(index);
  }

  setSeriesVisibility(index, visible) {
    this.setDatasetVisibility(index, visible);
  }
  ...
  _resize(width, height) {
    ...
    this._aspectRatio = this.aspectRatio;
    const changed = this._renderer.resize(newSize.width, newSize.height, newRatio);
    this.currentDevicePixelRatio = this._renderer.type === 'canvas' ? newRatio : 1;
    if (!changed) {
      return false;
    }
  ...
  _updateMetasets() {
    ...
    const numData = this._data.datasets.length;
  ...
  _removeUnreferencedMetasets() {
    const {_metasets: metasets, _data: {datasets}} = this;
  ...
  /**
	 ...
	 */
  _resetElements() {
    each(this._data.datasets, (dataset, datasetIndex) => {
  ...
  update(mode) {
    ...
    let minPadding = 0;
    for (let i = 0, ilen = this._data.datasets.length; i < ilen; i++) {
  ...
  _getUniformDataChanges() {
    ...
    this._dataChanges = [];
    const datasetCount = this._data.datasets.length;
  ...
  _drawDataset(meta) {
    ...
    this._renderer.beginDataset(meta.index, clip);

    meta.controller.draw();

    this._renderer.endDataset(clip);
  ...
  _stop() {
    ...
    for (i = 0, ilen = this._data.datasets.length; i < ilen; ++i) {
      this._destroyDatasetMeta(i);
    }
  }
   ...
   * @param {import('./chart.lifecycle.js').filterCallback} [filter] - Filtering function for limiting which plugins are notified
   ...
   * @param {import('../shared/types/index.js').ActiveElement[]} lastActive - Previously active elements
   * @param {boolean} inChartArea - Is the event inside chartArea
   * @param {boolean} useFinalPosition - Should the evaluation be done with current or final (after animation) element positions
   * @returns {import('../shared/types/index.js').ActiveElement[]} - The active elements
   * @pravate
   */
