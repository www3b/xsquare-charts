// @ts-nocheck
import {isCanvasPaint, isRendererNeutralPaint} from '../paint.js';
...
function createSvgElement(chart, name) {
  const root = chart.$chartjsSvgRoot || (chart._renderer && chart._renderer.root);
...
export function getOrCreateSvgRoot(chart) {
  ...
  if (!root) {
    if (chart._renderer && chart._renderer.type === 'svg') {
      root = chart._renderer.root;
