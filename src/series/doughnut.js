...
export default class DoughnutController extends DatasetController {
  ...
  _getRotationExtents() {
    ...
    for (let i = 0; i < this.chart._data.datasets.length; ++i) {
  ...
  getLabelAndValue(index) {
    const meta = this._cachedMeta;
    const chart = this.chart;
    const labels = chart._data.labels || [];
  ...
  getMaxBorderWidth(arcs) {
    ...
    if (!arcs) {
      ...
      for (i = 0, ilen = chart._data.datasets.length; i < ilen; ++i) {
  ...
  _getRingWeight(datasetIndex) {
    return Math.max(valueOrDefault(this.chart._data.datasets[datasetIndex].weight, 1), 0);
  }
  ...
  _getVisibleDatasetWeightTotal() {
    return this._getRingWeightOffset(this.chart._data.datasets.length) || 1;
  }
}
