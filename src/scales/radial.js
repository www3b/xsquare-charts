...
import {HALF_PI, TAU, toDegrees, toRadians, _normalizeAngle, PI} from '../shared/math.js';
import LinearScaleBase from './linear-base.js';
import Ticks from './scale.ticks.js';
import {valueOrDefault, isArray, isFinite, callback as callCallback, isNullOrUndef} from '../shared/core.js';
import {createContext, toFont, toPadding, toTRBLCorners} from '../shared/options.js';
...
function measureLabelSize(scale, font, label) {
  ...
  for (const line of label) {
    if (!isNullOrUndef(line)) width = Math.max(width, scale.chart._renderer.measureText(line, font.string));
  }
  return {
    w: width,
...
export default class RadialLinearScale extends LinearScaleBase {
  ...
  getRadialTickDrawItems() {
    ...
    this.ticks.forEach((tick, index) => {
      ...
      const padding = toPadding(opts.backdropPadding);
      const width = opts.showLabelBackdrop ? this.chart._renderer.measureText(tick.label, font.string) : 0;
  ...
  /**
	 ...
	 */
  drawBackground() {
    this.chart._renderer.drawRadialScale(this, 'background');
  }
  ...
  /**
	 ...
	 */
  drawGrid() {
    this.chart._renderer.drawRadialScale(this, 'grid');
  }
  ...
  /**
	 ...
	 */
  drawLabels() {
    this.chart._renderer.drawRadialScale(this, 'labels');
  }
